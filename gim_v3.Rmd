...
```{r}
sdf <- read_sf("data/sample_v3.geojson")
```
...
```{r}
final.features.df <- training.df %>% dplyr::select(TLABEL, Y, LC_Type2, X, pet, EVI, Tveg_tavg, Psurf_f_inst, NDVI, tmmx, Albedo_inst, pdsi, vs)
...
We get a best kappa value of 0.48 with mtry=8.  We will select these features, based on the feature importance and above experimentation.
